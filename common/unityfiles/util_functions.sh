...
ak2() {
  ...
  sed -i -e "s|<INSTALLER>|$INSTALLER|" -e "s|<OUTFD>|$OUTFD|" -e "s|<BOOTMODE>|$BOOTMODE|" -e "s|<SLOT>|$SLOT|" -e "s|<MAGISK>|$MAGISK|" -e "s|<DEBUG>|$DEBUG|" -e "s|<MODID>|$MODID|" $INSTALLER/common/anykernel.sh
  ...
  if [ -f "$INSTALLER/common/unityfiles/tools/busybox" ]; then
    ...
    local BB=$INSTALLER/common/unityfiles/tools/busybox-$BBABI
  fi
  chmod 755 $BB
